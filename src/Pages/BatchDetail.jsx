    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-400 font-medium animate-pulse">Decrypting Batch Data...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#FAFAFA] text-slate-900 pb-20">
        ...
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden md:block text-right">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Traceability Status</p>
      ...
      <main className="max-w-6xl mx-auto px-6 pt-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-16">
          <div className="lg:col-span-8">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-emerald-600 font-mono text-sm font-bold">{batch.batchId}</span>
            </div>
            ...
            <p className="text-xl text-slate-500 leading-relaxed max-w-2xl font-medium">
              Harvested from the pristine slopes of {batch.origin?.state}, this batch represents our commitment to absolute purity and traditional methods.
            </p>
          </div>
             ...
             {batch.isVerified && (
                <div className="bg-white border-2 border-emerald-500 p-6 rounded-[2rem] shadow-lg shadow-emerald-50 rotate-2 hover:rotate-0 transition-transform cursor-help group">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-xs font-black text-slate-400 uppercase">Authenticity</p>
                      <h4 className="text-lg font-black text-slate-900">100% Original</h4>
                    </div>
                  </div>
                </div>
             )}
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            <section className="bg-white border border-slate-200 rounded-[3rem] p-10 shadow-sm">
              <div className="flex items-center gap-4 mb-10">
                <div className="p-3 bg-slate-100 rounded-2xl"><History className="text-slate-600"/></div>
                <h3 className="text-xl font-black uppercase tracking-widest text-slate-800">The Journey</h3>
              </div>
                  ...
                  <div key={index} className="relative flex items-start gap-10 group">
                    ...
                    <div className="pl-14">
                      <time className="text-[11px] font-black text-emerald-600 uppercase mb-1 block">
                        {step.date ? new Date(step.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : "Process Phase"}
                      </time>
            ...
            {batch.images?.length > 0 && (
              <section className="space-y-6">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-sm font-black uppercase tracking-widest text-slate-400">Field Photography</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {batch.images.map((img, i) => (
                    <div key={i} className="overflow-hidden rounded-[2.5rem] bg-slate-200 aspect-[4/3] border border-slate-100 shadow-sm">
                      <img src={img} alt="Batch record" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
                    </div>
                  ))}
                </div>
              </section>
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div className="bg-slate-900 text-white rounded-[2.5rem] p-8 relative overflow-hidden shadow-2xl">
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <h3 className="text-xs font-black uppercase tracking-widest text-emerald-400">Geographic Origin</h3>
                </div>
                
                <div className="space-y-8">
                  <div>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Source Village</p>
                    <p className="text-2xl font-bold">{batch.origin?.location}</p>
                    <p className="text-slate-400 font-medium">{batch.origin?.state}, {batch.origin?.country}</p>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-6">
                    <div>
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Altitude</p>
                      <p className="text-lg font-bold flex items-center gap-2">
                        <Mountain size={16} className="text-emerald-400"/> {batch.origin?.altitude}m
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Coordinates</p>
                      <p className="text-sm font-mono text-slate-300">{batch.origin?.lat}°, {batch.origin?.lng}°</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Master Cultivator</p>
                  <h4 className="text-xl font-black text-slate-900">{batch.farmer?.name}</h4>
                </div>
              </div>
              <div className="space-y-3">
                 <div className="flex justify-between items-center text-sm">
                    <span className="text-slate-400 font-bold">Community</span>
                    <span className="text-slate-900 font-black">{batch.farmer?.village}</span>
                 </div>
              </div>
            </div>
            ...
            {/* SPECS LIST */}
            <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6">Batch Profile</h4>
              <div className="space-y-5">
                <DataRow icon={<Calendar size={16}/>} label="Harvest Season" value={batch.harvestSeason} />
                <DataRow icon={<Droplets size={16}/>} label="Processing" value={batch.processingMethod} />
                <DataRow icon={<Package size={16}/>} label="Packaging" value={batch.packagingType} />
                <DataRow icon={<Info size={16}/>} label="Total Yield" value={`${batch.totalUnits} Units`} />
              </div>
            </div>
            ...
            {batch.videoUrl && (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[10px] font-bold uppercase opacity-70">Visual Record</p>
        ...
        <section className="mt-20 border-t border-slate-200 pt-16 text-center max-w-3xl mx-auto">
          ...
          <h3 className="text-3xl font-black text-slate-900 mb-6">Behind the Batch</h3>
...
function DataRow({ icon, label, value }) {
  return (
    <div className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
      ...
      <span className="text-sm font-black text-slate-900 capitalize">{value}</span>
    </div>
  );
}