...
import { 
  ...
  ChevronRight, Search, Activity, Store, X, 
  Clock, CreditCard, Tag, Calendar, BarChart3, HardDrive, Layers
} from "lucide-react";
...
export default function AdminStore() {
  ...
  const initialForm = {
    ...
    autoOpenClose: 0, holidayDates: [],
    inventoryType: "store-based",
    priceMultiplier: 1,
    permissions: { canManageOrders: 1, canManageInventory: 1, canViewReports: 1, canEditStore: 0 },
    priority: 0, tags: "", zone: "",
    posSystemId: "", warehouseId: "",
    banner: "", notes: ""
  };
          ...
          <div className="relative bg-white w-full max-w-4xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-8">
              <div className="max-h-[50vh] overflow-y-auto pr-4 custom-scrollbar">
                ...
                {activeTab === "basic" && (
                  <div className="space-y-4 animate-in fade-in">
                    <div className="grid grid-cols-2 gap-4">
                      <Input label="Store Name" name="name" value={form.name} onChange={handleChange} placeholder="Main Outlet" />
                      <Input label="Store Code (UID)" name="code" value={form.code} onChange={handleChange} placeholder="SH-001" />
                    </div>
                ...
                {activeTab === "address" && (
                  <div className="space-y-4 animate-in fade-in">
                    <Input label="Address Line 1" name="address.line1" value={form.address.line1} onChange={handleChange} />
                    <Input label="Address Line 2" name="address.line2" value={form.address.line2} onChange={handleChange} />
                    <div className="grid grid-cols-3 gap-4">
                ...
                {activeTab === "services" && (
                  <div className="space-y-6 animate-in fade-in">
                    ...
                    <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100">
                      <ServiceToggle icon={<Truck />} title="Delivery Service" name="services.delivery.enabled" checked={form.services.delivery.enabled} onChange={handleChange} />
                      <div className="grid grid-cols-3 gap-4 mt-4">
                        ...
                        <Input label="Charge (₹)" type="number" name="services.delivery.charge" value={form.services.delivery.charge} onChange={handleChange} />
                        <Input label="Min Order for Free (₹)" type="number" name="services.delivery.freeDeliveryAbove" value={form.services.delivery.freeDeliveryAbove} onChange={handleChange} />
                      </div>
                    </div>
                    {/* Pickup Section */}
                    <div className="p-6 bg-emerald-50/30 rounded-[2rem] border border-emerald-100/50">
                      <ServiceToggle icon={<Package />} title="Pickup Service" name="services.pickup.enabled" checked={form.services.pickup.enabled} onChange={handleChange} color="emerald" />
                      <div className="grid grid-cols-2 gap-4 mt-4">
                        ...
                        <Input label="Max Orders / Slot" type="number" name="services.pickup.maxOrdersPerSlot" value={form.services.pickup.maxOrdersPerSlot} onChange={handleChange} />
                      </div>
                    </div>
                    {/* Express Section */}
                    <div className="p-6 bg-blue-50/30 rounded-[2rem] border border-blue-100/50">
                      <ServiceToggle icon={<Activity />} title="Express Delivery" name="services.expressDelivery.enabled" checked={form.services.expressDelivery.enabled} onChange={handleChange} />
                      <div className="grid grid-cols-2 gap-4 mt-4">
                        <Input label="Express Charge" type="number" name="services.expressDelivery.charge" value={form.services.expressDelivery.charge} onChange={handleChange} />
                        <Input label="Time (Mins)" type="number" name="services.expressDelivery.timeMinutes" value={form.services.expressDelivery.timeMinutes} onChange={handleChange} />
                      </div>
                    </div>
                  </div>
                )}
                ...
                {activeTab === "timing" && (
                  <div className="space-y-3 animate-in fade-in">
                    <div className="flex justify-between items-center px-4 py-2 bg-slate-100 rounded-xl mb-4">
                      <span className="text-xs font-black uppercase text-slate-500">Day</span>
                      <span className="text-xs font-black uppercase text-slate-500">Status & Hours</span>
                    </div>
                    {form.openingHours.map((slot, index) => (
                      <div key={slot.day} className="flex items-center gap-4 p-4 bg-white border border-slate-100 rounded-2xl shadow-sm">
                        <span className="w-20 text-sm font-bold text-slate-700">{slot.day}</span>
                        <button 
                          type="button"
                          onClick={() => {
                  ...
                  <div className="space-y-6 animate-in fade-in">
                    <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-4">
                        <Select label="Inventory System" name="inventoryType" value={form.inventoryType} onChange={handleChange}>
                          <option value="store-based">Store-Based (Local)</option>
                          <option value="central">Central (Cloud)</option>
                        </Select>
                        <Input label="Price Multiplier" type="number" step="0.1" name="priceMultiplier" value={form.priceMultiplier} onChange={handleChange} />
                        <Input label="Priority Rank" type="number" name="priority" value={form.priority} onChange={handleChange} />
                      </div>
                      <div className="p-6 bg-slate-900 rounded-[2rem] text-white">
                        <p className="text-[10px] font-black uppercase text-slate-400 mb-4 flex items-center gap-2"><Shield size={14}/> Node Permissions</p>
                        <div className="space-y-4">
                          {Object.keys(form.permissions).map(perm => (
                            <div key={perm} className="flex justify-between items-center group">
                              <span className="text-xs font-bold text-slate-300 group-hover:text-white transition-colors">{perm.replace(/([A-Z])/g, ' $1').trim()}</span>
                              <input 
                                type="checkbox" 
                                checked={form.permissions[perm] === 1} 
                                onChange={(e) => handleChange({target: { name: `permissions.${perm}`, value: e.target.checked ? 1 : 0}})}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       <Input label="POS System ID" name="posSystemId" value={form.posSystemId} onChange={handleChange} />
                       <Input label="Warehouse ID" name="warehouseId" value={form.warehouseId} onChange={handleChange} />
                    </div>
                    <div className="grid grid-cols-2 gap-4 items-center p-4 bg-white border border-slate-100 rounded-2xl">
                        <span className="text-sm font-bold text-slate-700">Auto Open/Close Automation</span>
                        <div className="flex justify-end">
                          <StatusToggle active={form.autoOpenClose} onClick={() => setForm({...form, autoOpenClose: form.autoOpenClose ? 0 : 1})} />
                        </div>
                    </div>
                    <textarea name="notes" value={form.notes} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-xs h-24 outline-none" placeholder="Internal logistics notes..." />
                  </div>
                )}
              ...
              <div className="mt-8 flex gap-3">
                <button type="button" onClick={closeModal} className="flex-1 px-6 py-4 rounded-2xl font-bold text-slate-500 hover:bg-slate-50 transition-colors">Discard</button>
...
const Input = ({ label, ...props }) => (
  <div className="w-full">
    ...
    <input {...props} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all" />
  </div>
);
