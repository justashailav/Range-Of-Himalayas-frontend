...
export default function AdminOrderDetailsView() {
  ...
  const handlePrint = async () => {
    ...
    <html>
      <head>
        ...
        <style>
          @page { size: auto; margin: 5mm; }
          body { font-family: "Poppins", sans-serif; font-size: 12px; color: #222; margin: 0; padding: 0; background: #fff; }
          .receipt { width: 280px; margin: auto; padding: 10px; }
          .header { text-align: center; border-bottom: 1px dashed #aaa; padding-bottom: 5px; }
          .header img { width: 60px; margin-bottom: 4px; }
          .brand { font-size: 15px; font-weight: bold; color: #2d572c; }
          .details { text-align: center; font-size: 11px; color: #555; margin-top: 2px; }
          .section { margin-top: 10px; border-bottom: 1px dashed #ccc; padding-bottom: 5px; }
          .section h3 { font-size: 12px; color: #2d572c; margin-bottom: 3px; }
          .item, .box-item { display: flex; justify-content: space-between; margin: 3px 0; font-size: 11px; }
          .item-name, .box-item-name { flex: 1; color: #333; }
          .totals { text-align: right; margin-top: 8px; font-size: 11px; }
          .qr { text-align: center; margin-top: 10px; }
          .qr img { width: 70px; height: 70px; }
          .thanks { text-align: center; font-size: 11px; color: #777; margin-top: 8px; }
          .signature { text-align: center; margin-top: 10px; font-size: 10px; }
          .signature-line { border-top: 1px solid #333; width: 120px; margin: 10px auto 3px; }
        </style>
      </head>
      <body>
        <div class="receipt">
          <div class="header">
            <img src="${logo}" alt="Logo" />
            <div class="brand">Range of Himalayas</div>
            <div class="details">Shimla, Himachal Pradesh<br>+91 6230867344</div>
          </div>
          <div class="section">
            <h3>Shipping To</h3>
            <p>${userName || orderDetails?.user?.name || "Customer"}</p>
            <p>${orderDetails?.addressInfo?.address || ""}, ${orderDetails?.addressInfo?.city || ""}</p>
            <p>Phone: ${orderDetails?.addressInfo?.phone || ""}</p>
          </div>
          <div class="section">
            <h3>Order Date (System)</h3>
            <p>${new Date(orderDetails.createdAt).toLocaleString('en-IN')}</p>
          </div>
          <div class="qr">
  ...
  if (!orderDetails) return <p className="text-center mt-20 text-gray-500">Loading...</p>;
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-semibold uppercase tracking-widest text-stone-500">
              Placed {getTimeElapsed(orderDetails.createdAt)}
            </span>
          </div>
          <button
            onClick={handlePrint}
            className="flex items-center gap-2 bg-stone-900 text-white px-6 py-3 rounded-xl hover:bg-stone-800 transition shadow-lg"
          >
        ...
        <section className="bg-white shadow-xl rounded-3xl border border-stone-100 p-8">
          <h2 className="text-2xl font-bold mb-8 text-stone-900 flex items-center gap-3">
            <FileText className="text-stone-400" /> Order Summary
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
            {[
              ["Reference ID", `#${orderDetails._id}`],
              ["Creation Date", new Date(orderDetails.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' })],
              ["Creation Time", new Date(orderDetails.createdAt).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true })],
              ["Total Revenue", `₹${orderDetails.totalAmount}`],
              ["Method", orderDetails.paymentMethod],
              ["Payment", orderDetails.paymentStatus],
            ].map(([label, value]) => (
              <div key={label} className="flex justify-between items-center border-b border-stone-50 py-3">
                <span className="text-sm font-medium text-stone-400 uppercase tracking-tighter">{label}</span>
                <span className="text-stone-900 font-semibold">{value || "-"}</span>
              </div>
            ))}

            <div className="flex justify-between items-center py-3">
              <span className="text-sm font-medium text-stone-400 uppercase tracking-tighter">Workflow Status</span>
              <Badge className={`px-4 py-1 rounded-full uppercase text-[10px] tracking-widest ${
                orderDetails.orderStatus === "delivered" ? "bg-green-500" : "bg-amber-500"
              }`}>
                {orderDetails.orderStatus}
              </Badge>
            </div>
          ...
          <form onSubmit={handleUpdateStatus} className="mt-10 flex gap-4">
            <select
              value={formData.status}
              onChange={(e) => setFormData({ ...formData, status: e.target.value })}
              className="flex-1 rounded-xl border border-stone-200 bg-stone-50 p-3 text-sm focus:ring-2 focus:ring-stone-900"
              required
            >
            ...
            <button className="bg-stone-900 text-white px-8 py-3 rounded-xl hover:bg-black transition-all">
              Update
            </button>
          </form>
        </section>
        ...
        {/* Shipping Information */}
        <section className="bg-white shadow-md rounded-3xl border border-stone-100 p-8">
          <h2 className="text-xl font-bold mb-6 text-stone-800">Logistic Destination</h2>
          {orderDetails?.addressInfo ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-xs text-stone-400 uppercase font-bold">Consignee</p>
                      <p className="text-stone-900 font-medium">{userName || "Customer Name"}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-stone-100 flex items-center justify-center">
                    ...
                    <div>
                      <p className="text-xs text-stone-400 uppercase font-bold">Contact</p>
                      <p className="text-stone-900 font-medium">{orderDetails.addressInfo.phone}</p>
                    </div>
                  </div>
               </div>
               <div className="space-y-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-xs text-stone-400 uppercase font-bold">Shipping Address</p>
                      <p className="text-stone-900 font-medium leading-relaxed">
                        {orderDetails.addressInfo.address}, {orderDetails.addressInfo.city}<br />
                        {orderDetails.addressInfo.state} - {orderDetails.addressInfo.pincode}
                      </p>
                    </div>
                  </div>
               </div>
            </div>
          ) : <p className="text-stone-400 italic">Address data not available.</p>}
        </section>

        {/* Custom Box Logic & Items List */}
        {orderDetails.boxes?.length > 0 && (
          <section className="bg-stone-900 text-white shadow-2xl rounded-3xl p-8">
            <h3 className="text-2xl font-light mb-8 flex items-center gap-3">
              <Box className="text-amber-400" /> Curated Custom Boxes
            </h3>
            <div className="space-y-6">
              {orderDetails.boxes.map((box, idx) => (
                <div key={idx} className="bg-stone-800/50 rounded-2xl p-6 border border-stone-700">
                  <p className="text-amber-400 text-xs font-bold uppercase tracking-widest mb-4">Pack Unit #{idx+1}</p>
                  <ul className="space-y-4">
                    {box.items?.map((item, i) => {
                       const product = productList?.find((p) => p._id === item.productId) || {};
                       return (
                         <li key={i} className="flex justify-between items-center text-sm">
                           <span className="text-stone-300">{product.title} <span className="text-stone-500">({item.size})</span></span>
                           <span className="font-mono text-amber-200">x{item.quantity}</span>
                         </li>
                       );
                    })}
                  </ul>
