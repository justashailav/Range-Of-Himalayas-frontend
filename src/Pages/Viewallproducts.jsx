      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {productList && productList.length > 0 ? (
          productList.map((item) => (
            <div
              key={item._id}
              className="relative group bg-white rounded-2xl shadow-md overflow-hidden hover:scale-[1.01] transition-transform duration-300"
            >
              <TopSelections
                product={item}
              ...
              <div className="absolute inset-0 flex items-center justify-center rounded-2xl pointer-events-none">
                <div className="absolute inset-0 bg-black/10 backdrop-blur-[1px] rounded-2xl transition-opacity duration-300 group-hover:opacity-30"></div>
