...
export default function Blog() {
  ...
  >
    Our <span className="font-light italic text-stone-400">Journal</span>
  </motion.h2>
  ...
  >
    Exploring the traditions, harvests, and soul of the Himalayan foothills.
  </motion.p>
</div>
...
>
  ...
  <motion.p
    className="text-center text-[#5C4033] max-w-xl mx-auto font-medium leading-relaxed italic"
  >
...
>
  {blogs?.map((b, idx) => {
    const shareUrl = b.slug ? `${window.location.origin}/blog/${b.slug}` : window.location.origin;
          ...
          <div className="space-y-4 px-2">
            <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-stone-400">
              <span className="flex items-center gap-1.5">
                ...
                {new Date(b.createdAt).toLocaleDateString("en-IN", { month: "short", day: "2-digit", year: "numeric" })}
              </span>
            ...
            <div className="flex items-center justify-between pt-4">
              <div className="flex items-center gap-6">
                {/* LIKE */}
                <button
                  onClick={(e) => { e.preventDefault(); handleLike(e, b._id); }}
                  className="flex items-center gap-1.5 text-stone-400 hover:text-red-500 transition-colors"
                >
                  <Heart size={16} fill={b.isLiked ? "currentColor" : "none"} className={b.isLiked ? "text-red-500" : ""} />
                  <span className="text-[10px] font-black">{b.likesCount || 0}</span>
                </button>

                {/* SHARE TRIGGER */}
                <div className="relative group/share">
                  <button
                    onClick={(e) => { e.preventDefault(); handleShareClick(e, b._id); }}
                  ...
                  <AnimatePresence>
                    {openShareId === b._id && (
                      ...
                      >
                        {[
                          { href: `https://twitter.com/intent/tweet?url=${encodedUrl}`, icon: <FaTwitter />, color: "hover:bg-sky-500" },
                          { href: `https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}`, icon: <FaFacebook />, color: "hover:bg-blue-600" },
                          { href: `https://t.me/share/url?url=${encodedUrl}`, icon: <FaTelegramPlane />, color: "hover:bg-sky-600" }
                        ].map((btn, i) => (
                          <a
                            key={i} href={btn.href} target="_blank" rel="noopener noreferrer"
...
>
  ...
  <div className="relative z-10 text-center space-y-8">
    <motion.div
      variants={fadeIn}
      className="inline-flex items-center gap-3 px-4 py-2 rounded-full border border-stone-700 bg-stone-800/50 backdrop-blur-md"
    >
      ...
      <span className="text-[10px] font-black text-stone-300 uppercase tracking-[0.3em]">Prelaunch Active</span>
    </motion.div>
    ...
    >
      First Taste of the <br />
      <span className="italic font-light text-stone-400">Himalayan Harvest</span>
    </motion.h2>

    <motion.p
      variants={fadeIn}
      custom={0.2}
      className="text-stone-400 text-sm md:text-base max-w-xl mx-auto leading-relaxed font-medium"
    >
      Our private orchards are nearing peak ripeness. Join the curated waitlist for 
      <span className="text-white mx-1 font-bold">priority access</span> 
      to the season's first pick, exclusive member pricing, and direct updates from the foothills.
    </motion.p>
    ...
    >
      ...
      >
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          className="group relative flex items-center gap-4 bg-white text-stone-900 px-12 py-5 rounded-2xl text-xs font-black uppercase tracking-[0.2em] transition-all duration-300 shadow-xl shadow-black/20"
        >
          <span>Join the Waitlist</span>
          <ArrowRight size={16} className="group-hover:translate-x-2 transition-transform duration-300 text-[#B23A2E]" />
        </motion.button>
      </motion.a>
    </motion.div>
    ...
    <motion.p 
      variants={fadeIn}
      custom={0.4}
      className="text-[9px] font-bold text-stone-600 uppercase tracking-widest pt-4"
    >
      Limited to 250 Seasonal Members
    </motion.p>
  </div>
</motion.section>
      <Footer />
