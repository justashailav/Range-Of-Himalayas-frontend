...
export default function Navbar() {
  ...
  <div className="flex items-center gap-2 sm:gap-4">
    ...
    <Sheet open={openCart} onOpenChange={setOpenCart}>
      ...
      <SheetContent side="right" className="sm:max-w-md bg-[#fdfcf7] p-0">
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-56 mt-3 bg-[#fdfcf7] border-stone-200">
          {/* ... keeping your existing dropdown items ... */}
      </DropdownMenuContent>
    </DropdownMenu>
    ...
    >
      <div className="flex flex-col items-end gap-1 group">
        <span className={`h-[1.5px] bg-white transition-all duration-500 ${isMenu ? "w-6 -rotate-45 translate-y-[5px]" : "w-6"}`} />
        <span className={`h-[1.5px] bg-white transition-all duration-500 ${isMenu ? "opacity-0 w-0" : "w-4"}`} />
        <span className={`h-[1.5px] bg-white transition-all duration-500 ${isMenu ? "w-6 rotate-45 -translate-y-[5px]" : "w-6"}`} />
      </div>
    </button>
  </div>
