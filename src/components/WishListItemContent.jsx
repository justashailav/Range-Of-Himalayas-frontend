...
export default function WishListItemContent() {
  ...
  const handleRemove = (item) => {
    dispatch(
      ...
      if (res?.success) toast.success("Item removed from wishlist");
  ...
  if (!wishListItems.length) {
    return (
      <div className="text-center py-20 text-gray-500 space-y-6">
        ...
        <h2 className="text-2xl font-semibold">Your Wishlist is Empty</h2>
        <p className="max-w-md mx-auto text-gray-600">
          Add products you love to your wishlist and move them to cart anytime.
        </p>
      </div>
    );
  }
    ...
    <div className="px-4 sm:px-6 lg:px-10 py-8 bg-[#FFF8E1] min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {wishListItems.map((item) => {
          ...
          if (item.size) {
            variant = product.sizes?.find(
              (s) => s.size === item.size
            );
          } else {
            variant = product.weights?.find(
              (w) => w.weight === item.weight
            );
          }

          const price = variant?.price ?? 0;
          const salesPrice = variant?.salesPrice ?? price;
          ...
          return (
            <div
              key={`${item.productId}-${item.size}-${item.weight}`}
              className="relative bg-white rounded-2xl shadow-md hover:shadow-xl transition-all"
            >
              {/* REMOVE */}
              <button
                onClick={() => handleRemove(item)}
                className="absolute top-3 right-3 w-9 h-9 bg-gray-100 rounded-full hover:bg-red-500 hover:text-white transition"
              >
                <Trash2 className="w-4 h-4" />
              </button>
              ...
              <div className="p-5 space-y-4">
                ...
                <Button
                  ...
                  className="w-full bg-[#F08C7D] hover:bg-[#f39b8d]"
                >
