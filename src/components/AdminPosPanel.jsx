...
import {
  ...
  Package, CreditCard, X, Store, Zap
} from "lucide-react";
...
export default function AdminPOSPanel({ storeId }) {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setCart([]);
      hasAutoAdded.current = false; // Reset for next potential action
      dispatch(fetchStoreProducts(storeId));
    } catch (err) {
      console.error(err);
    }
        ...
        <header className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-blue-600 mb-1">
              <Store size={14} />
              <span className="text-[10px] font-black uppercase tracking-[0.2em]">Live Inventory</span>
            </div>
            <h1 className="text-4xl font-black tracking-tighter text-slate-900">
              Terminal <span className="text-slate-300 italic font-light lowercase">pos</span>
            </h1>
          </div>
              ...
              p.variants.map((v, i) => (
                ...
                >
                  ...
                  <div className="px-2 pb-2">
                    <h3 className="text-sm font-bold text-slate-800 leading-tight truncate">{p.title}</h3>
                    <p className="text-[10px] font-bold text-slate-400 uppercase mb-2">{v.weight}</p>
      ...
      <div className="w-full lg:w-[420px] bg-white lg:m-4 lg:rounded-[3rem] border border-slate-100 flex flex-col shadow-2xl relative">
        <div className="p-8 pb-4">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-black tracking-tight text-slate-900">Current Order</h2>
        ...
        <div className="flex-1 overflow-y-auto px-8 space-y-4">
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-300 py-20">
              ...
              <p className="text-xs font-black uppercase tracking-widest text-center">Terminal is Idle<br/><span className="font-medium text-slate-400 lowercase">Select items to begin</span></p>
            </div>
          ) : (
            cart.map((item) => (
              <div key={item.sku} className="flex gap-4 group animate-in slide-in-from-right-4 duration-300">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="text-xs font-black text-slate-800 leading-none mb-1">{item.title}</h3>
                      <p className="text-[10px] font-bold text-slate-400 uppercase">{item.weight} • SKU: {item.sku}</p>
                    </div>
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <div className="flex items-center bg-slate-50 rounded-xl p-1 border border-slate-100">
                      <button onClick={() => updateQty(item.sku, "dec")} className="p-1 hover:text-blue-600"><Minus size={12}/></button>
                      <span className="text-xs font-black w-8 text-center text-slate-700">{item.quantity}</span>
                      <button onClick={() => updateQty(item.sku, "inc")} className="p-1 hover:text-blue-600"><Plus size={12}/></button>
                    </div>
        ...
        <div className="p-8 bg-slate-50 lg:rounded-b-[3rem] border-t border-slate-100">
          <div className="space-y-3 mb-8">
            <div className="flex justify-between text-xs font-bold text-slate-400 uppercase tracking-widest">
              <span>Subtotal</span>
              <span className="text-slate-900 font-mono">₹{total.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-xs font-bold text-slate-400 uppercase tracking-widest">
              <span>Handling</span>
            ...
            <div className="pt-4 border-t border-slate-200 flex justify-between items-end">
              <span className="text-sm font-black uppercase text-slate-900">Payable Amount</span>
              <span className="text-3xl font-black text-blue-600 tracking-tighter italic">₹{total.toFixed(2)}</span>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full bg-slate-900 text-white py-5 rounded-2xl flex items-center justify-center gap-4 font-black uppercase tracking-[0.2em] text-[10px] shadow-2xl shadow-slate-300 hover:bg-blue-600 transition-all active:scale-95 disabled:bg-slate-200 disabled:shadow-none"
          >
