...
export default function ProductTile({
  ...
  <div className="relative h-52 overflow-hidden">
    ...
    />
    {/* Dynamic Stock Badge */}
    <div className={`absolute top-4 right-4 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest backdrop-blur-md shadow-sm ${
      stock < 10 ? "bg-red-50 text-red-600 border border-red-100" : "bg-white/90 text-stone-600 border border-stone-100"
    }`}>
  ...
  <div className="p-6">
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        {hasSize && (
          <div className="flex flex-col gap-1">
            <label className="text-[9px] font-bold text-stone-400 uppercase ml-1">Grade/Size</label>
            <select
              value={selectedSize}
              onChange={(e) => {
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[9px] font-bold text-stone-400 uppercase ml-1">Net Weight</label>
          <select
            value={selectedWeight}
            onChange={(e) => setSelectedWeight(e.target.value)}
            className="appearance-none bg-stone-50 border border-stone-100 text-stone-700 text-xs rounded-xl px-3 py-2 outline-none focus:border-orange-200 transition-colors cursor-pointer"
          >
            {getWeights(selectedSize).map((weight) => <option key={weight} value={weight}>{weight}</option>)}
          </select>
        </div>
      </div>
    )}
    ...
    <div className="flex items-baseline gap-3 mb-6 bg-stone-50/50 p-3 rounded-2xl border border-stone-50">
      <div className="flex flex-col">
        <span className="text-[9px] font-bold text-stone-400 uppercase">Current MSRP</span>
        <div className="flex items-center gap-2">
           <span className="text-xl font-serif font-bold text-stone-900">₹{finalPrice.toFixed(2)}</span>
           <span className="text-xs text-stone-400 line-through decoration-orange-500/30">₹{price.toFixed(2)}</span>
        </div>
      </div>
    </div>
